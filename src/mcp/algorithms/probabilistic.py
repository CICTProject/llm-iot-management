...
def probabilistic_spatially_optimized_activation(
    ...
    for node in selected_devices:
        ...
        activation_plan.append({
            ...
            "detection_accuracy_percent": node.detection_accuracy * (activation_duration / (t_base * gamma_t)) if (t_base * gamma_t) > 0 else 0.0,
    ...
    metrics = {
        ...
        "activation_duration_seconds": activation_duration,
        "avg_detection_accuracy_percent": np.mean([n.detection_accuracy * (activation_duration / (t_base * gamma_t)) if (t_base * gamma_t) > 0 else 0.0 for n in selected_devices]) if selected_devices else 0.0,
        "energy_efficiency_percent": 100 - (total_energy / sum(n.energy_consumption for n in nodes) * 100) if nodes else 0.0,
